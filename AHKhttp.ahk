...
class HttpResponse
{
    ...
    Generate() {
        ...
        buffer := new DataBuffer((StrLen(headers) * 2) + length)
    ...
    SetBody(ByRef body, length) {
        this.body := new DataBuffer(length)
        this.body.Write(&body, length)
        this.headers["Content-Length"] := length
    }
    
    SetBodyText(text) {
        this.body := DataBuffer.FromString(text)
...
class DataBuffer
{
    ...
    FromString(str, encoding = "UTF-8") {
        length := DataBuffer.GetStrSize(str, encoding)
        buffer := new DataBuffer(length)
        buffer.WriteStr(str)
        return buffer
    }
